...
export async function GET(request: NextRequest) {
    ...
    if (query) {
        ...
        const busqueda = `query=${encodeURIComponent(query)}&include_adult=false&language=es-MX&page=${page}`
    ...
    if (!genre) {
        ...
        const data = await fetchDataNoStore(option, `page=${page}&language=es-MX`)
        return NextResponse.json(data)
    }

    // CATEGORIA
    const data = await fetchDataNoStore(`/discover/${type}`, `with_genres=${genre}&page=${page}&language=es-MX`)
    return NextResponse.json(data)
}
