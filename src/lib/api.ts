...
// Buscador 
export const fetchSearch = async (type: string, searchParams: QueryProps, page: number = 1) => {
    ...
    if (search) {
        ...
        try {
            const response = await fetchDataNoStore(path, `query=${encodeURIComponent(search)}&include_adult=false&language=es-ES`);

            const filteredResults = response.results.filter((result: Media) => (
    ...
    // Categoria
    return fetchDataNoStore(`/discover/${type}`, `with_genres=${genre}&language=es-MX`);
}